          ...
          <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <View style={styles.form}>
                  ...
                  {route?.params?.pop?.attributes?.image?.data?.length > 0 ? (
                    <Image
                      ...
                      source={{
                        uri:
                          API_URL +
                          route.params.pop.attributes.image.data[0].attributes
                            .url,
                      }}
                      resizeMode="cover"
                    />
              ...
              <Text style={basic.label}>
                Série<Text style={basic.mandatory}>*</Text>
              </Text>
              ...
              {serie && serie.id == -1 && (
                <>
                  <Text style={basic.label}>
                    Autre serie<Text style={basic.mandatory}>*</Text>
                  </Text>
                  <TextInput
                    style={basic.input}
                    autoCapitalize={"none"}
              ...
              <Text style={basic.label}>
                Nom du popmart<Text style={basic.mandatory}>*</Text>
              </Text>
              <TextInput
                style={basic.input}
              ...
              <View style={styles.radioBox}>
                ...
                >
                  <RadioButton.Item
                    color={color.pink}
                    labelStyle={{ fontFamily: "rbt-Bold" }}
                    label="Je veux l'échanger"
                    value="change"
                  />
                  <RadioButton.Item
                    color={color.pink}
                    labelStyle={{ fontFamily: "rbt-Bold" }}
                    label="Je cherche ce modèle"
                    value="looking"
                  />
                  <RadioButton.Item
                    color={color.pink}
                    labelStyle={{ fontFamily: "rbt-Bold" }}
                    label="Elle est réservé"
                    value="booked"
                  />
                </RadioButton.Group>
              </View>
                ...
                onPress={() => {
                  ...
                  data.append(
                    ...
                    JSON.stringify({
                      name,
                      note,
                      serie: serie.id == -1 ? other : serie.attributes.name,
                  ...
                  updatePop({ data: data, id: route?.params?.pop.id })
                    .unwrap()
                    .then((res) => {
                      console.log("res ", res)
                    ...
                    .catch((err) => {
                      ...
                      Alert.alert(
                        "Erreur",
                        "Erreur de serveur, veuillez réessayer ultérieurement.",
                        [{ text: "OK" }]
                      );
                    });
                  cleanVariables();
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  Alert.alert(
                    "Supression de popmart",
                    "Êtes vous sûre de vouloir supprimer ce popmart ?",
                    [
                      {
                        text: "Annuler",
                        style: "cancel",
                      },
                      {
                        ...
                        onPress: () => {
                          setLoading(true);
                          removePop({ id: route?.params?.pop.id })
                            .unwrap()
                            .then((res) => {
                              setLoading(false);
                              cleanVariables();
                              Alert.alert(
                                "Action réussi",
                                "Votre popmart a bien été supprimé",
                                [
                                  {
                                    text: "Ok",
                                    onPress: () => {
